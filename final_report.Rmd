...
```{r bonus}
linate.lm <- lm(flight_data$Flights ~ flight_data$Calls.and.SMS.Activity + flight_data$Internet.Activity + flight_data$Twitter.Activity)
model_data <- augment(linate.lm)

# Won't work because no 'Days' in model_data
# ggplot(model_data, aes(Days, .resid)) + geom_boxplot(aes(fill=Days)) + ylab('Residuals')
aggregation <- aggregate(Flights ~ Calls.and.SMS.Activity + Internet.Activity + Twitter.Activity, flight_data, var)
