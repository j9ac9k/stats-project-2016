...
p1 <- ggplot(model_data, aes(Day, .resid)) + geom_boxplot(aes(fill=Day)) + ylab('Residuals') + 
  ggtitle('Residuals of total model') + 
  theme(panel.background = element_blank(),
        panel.border = element_rect(colour='black', fill=NA, size=1))

# Plot residuals of only Twitter
linate_twitter.lm <- lm(Twitter.Activity ~ Day, data=bonus_data)
model_twitter <- augment(linate_twitter.lm)
p2 <- ggplot(model_twitter, aes(Day, .resid)) + geom_boxplot(aes(fill=Day)) + ylab('Residuals') + 
  ggtitle('Residuals of Twitter data') + 
  theme(panel.background = element_blank(),
        panel.border = element_rect(colour='black', fill=NA, size=1))

grid.arrange(p1, p2, ncol=1, nrow=2, respect=FALSE)
```
