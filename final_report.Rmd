...
bonus_data$Day <- factor(bonus_data$Day, levels =c('Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'))
...
linate_call.lm <- lm(Calls.and.SMS.Activity ~ Day, data=bonus_data)
linate_internet.lm <- lm(Internet.Activity ~ Day, data=bonus_data)
linate_twitter.lm <- lm(Twitter.Activity ~ Day, data=bonus_data)

model_calls <- augment(linate_call.lm)
model_internet <- augment(linate_internet.lm)
model_twitter <- augment(linate_twitter.lm)

ggplot(model_calls, aes(Day, .resid)) + geom_boxplot(aes(fill=Day)) + ylab('Residuals')
ggplot(model_internet, aes(Day, .resid)) + geom_boxplot(aes(fill=Day)) + ylab('Residuals')
ggplot(model_twitter, aes(Day, .resid)) + geom_boxplot(aes(fill=Day)) + ylab('Residuals')
```
```{r anova_tukey}
bonus_data.lm <- lm(Calls.and.SMS.Activity + Internet.Activity + Twitter.Activity ~ Day, data=bonus_data)
anova(bonus_data.lm)

bonus_mcp <- glht(bonus_data.lm,mcp(Day = "Tukey"))
confint(bonus_mcp)
summary(bonus_mcp, test = univariate())
```
...
bonus_data_weekday.lm <- lm(Calls.and.SMS.Activity + Internet.Activity + Twitter.Activity ~ Day, data=bonus_data_weekday)

# ANOVA
anova(bonus_data_weekday.lm)

# Tukey
# bonus_weekday_mcp <- glht(bonus_data_weekday.lm,mcp(Day = "Tukey"))
# confint(bonus_weekday_mcp)
# summary(bonus_weekday_mcp, test = univariate())
```
